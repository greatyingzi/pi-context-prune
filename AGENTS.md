...
- **`flushPending(ctx, options?)`** — summarizes + indexes all unsummarized batches via a single structured LLM call (`summarizeAllBatches`) by default, or sequentially (one call per batch) when `options.onProgress` is provided. Accepts `FlushOptions`: `{ delivery?, onProgress?, onBatchTextProgress?, previewedBatches? }`. When `previewedBatches` is set the internal capture step is skipped (avoids double-capture from `/pruner now`). The structured path sends all batches in one request with `<turn index="N">` XML delimiters and parses JSON output; falls back to parallel-per-batch if JSON parsing fails. While summarization is running, `onBatchTextProgress` receives the streamed summary character count for the active batch so callers can surface live progress in richer UIs (the `/pruner now` overlay and `context_prune` tool updates), while the footer itself stays at the simpler `prune: summarizing…` state. **Small-batch filtering**: batches with raw content < 800 chars (`MIN_BATCH_RAW_CHARS_TO_SUMMARIZE`) skip LLM summarization entirely — the frontier advances past them, no index entries are created, and the tool results remain in context. A single aggregate `info` notification lists all skipped turns. Returns a typed `FlushResult` with reason `"flushed"`, `"skipped-oversized"`, or `"skipped-small"`.
...
- **`SummarizeBatchesOptions`** — `{ onBatchTextProgress? }` — optional hooks for multi-batch summarization (both structured single-call and parallel fallback) so callers can surface live text progress in the footer or overlay.
...
- **`serializeAllBatchesForStructured(batches)`** — renders batches with `<turn index="N">` XML delimiters for the structured (single-call) summarization path. Reuses `serializeBatchForSummarizer` for each turn's inner content. Used by `summarizeAllBatches`.
...
- **`summarizeBatches(batches, config, ctx, options?)`** — summarizes multiple `CapturedBatch` objects via one parallel LLM call per batch. Kept as fallback for when `summarizeAllBatches` fails to parse JSON. If only one batch, delegates to `summarizeBatch`.
- **`summarizeAllBatches(batches, config, ctx, options?)`** — summarizes all batches in a **single LLM call** using structured JSON output. All batches are serialized with `<turn index="N">` XML delimiters, sent to the LLM in one request, and the JSON response is parsed to extract per-turn summaries. Reduces N parallel calls to 1. Falls back to `summarizeBatches` (parallel) if JSON parsing fails. Token usage is attributed proportionally: input tokens by batch text-size ratio, output tokens and cost split evenly.
...
| `MultiBatchLoaderOverlay` for `/pruner now` | Shows per-batch spinner rows with live streamed summary-character counts, then checks each row off as its LLM call completes — gives concrete progress feedback instead of a single static "summarizing…" message. Sequential LLM calls are used only for this user-triggered path; auto-flush paths use the structured single-call approach (`summarizeAllBatches`) for efficiency |
| Structured single-call summarization (`summarizeAllBatches`) | Default path for auto-flush: all batches serialized with `<turn index="N">` XML delimiters, sent to the LLM in one request, JSON output parsed to extract per-turn summaries. Reduces N parallel calls to 1. Falls back to parallel-per-batch if JSON parsing fails. Token usage attributed proportionally across batches |
...
| Small-batch skip (`MIN_BATCH_RAW_CHARS_TO_SUMMARIZE = 800`) | Batches with < 800 chars of raw tool-result text are too small for LLM summarization to be worthwhile — the summary would cost more tokens than it saves. These batches advance the prune frontier without LLM calls, index entries, or session summaries. A single aggregate notification tells the user which turns were skipped. |
