...
import { extractKeywords, tagMatchGraph, serializeForInjection } from "./src/knowledge-matcher.js";
...
export default function (pi: ExtensionAPI) {
  ...
  pi.on("context", async (event, ctx) => {
    ...
    // Preflight knowledge retrieval: match knowledge graph tags to user prompt.
    // Tags were generated by the LLM during flushPending summarization,
    // so this is purely code-level matching — zero extra LLM calls.
    ...
    if (graphSize > 0) {
      try {
        ...
        if (lastUserMsg) {
          ...
          if (userText) {
            ...
            if (keywords.length > 0) {
              const candidates = tagMatchGraph(knowledgeGraph.getGraph(), keywords);
              if (candidates.length > 0) {
                // Build set of file paths that already have unpruned summaries
                ...
                const selected = candidates.slice(0, 5).map((c) => c.entry);
                const knowledgeText = serializeForInjection(selected, pathsWithActiveSummary);
                if (knowledgeText) {
                  // Append to last toolResult if available, otherwise to user message
        ...
        // Non-critical: knowledge matching is supplementary
      }
    }
